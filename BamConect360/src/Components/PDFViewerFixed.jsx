...
export default function PDFViewer() {
	...
	const handleStarClick = async (starValue) => {
		...
		try {
			...
			const response = await fetch(
				`${API_BASE_URL}/pdf/${pdfData._id}/rating`,
				{
					method: "POST",
					headers: {
	...
	const loadRatingData = async (pdfId) => {
		try {
			const response = await fetch(`${API_BASE_URL}/pdf/${pdfId}/rating`);
			if (response.ok) {
					...
					>
						...
						<div
							style={{
								...
								fontSize: "16px",
								color: "#374151",
								maxHeight: showFullContent ? "none" : "400px",
						...
						>
							...
							>
								{showFullContent
									? pdfData.content
									: truncateContent(pdfData.content, 1000)}
							</pre>

							{!showFullContent &&
								pdfData.content &&
								pdfData.content.length > 1000 && (
									<div
										style={{
											position: "absolute",
											bottom: 0,
						...
						>
							{pdfData.content && pdfData.content.length > 1000 && (
								<button
									style={{
										background: "#f3f4f6",
										color: "#6b7280",
